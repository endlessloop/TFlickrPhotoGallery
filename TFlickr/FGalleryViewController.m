...
- (void)loadView
{
	...
	_shareButton = [[UIBarButtonItem alloc] initWithImage:shareIcon style:UIBarButtonItemStylePlain target:self action:@selector(share)];
	// add prev next to front of the array
	[_barItems insertObject:_nextButton atIndex:0];
    [_barItems insertObject:_shareButton atIndex:0];
...
- (void) share{
    ...
    {     
        ...
        if (photoView != nil)
        {
            activityItems = [[NSArray alloc]initWithObjects:SHARE_TEXT, photoView.imageView.image, nil];
        }
        
        UIActivityViewController *activityVC;
         activityVC = [[[UIActivityViewController alloc]initWithActivityItems:activityItems applicationActivities:nil] autorelease];
        activityVC.excludedActivityTypes = @[UIActivityTypeAssignToContact,UIActivityTypePostToWeibo,UIActivityTypeMessage,UIActivityTypeCopyToPasteboard,UIActivityTypeAssignToContact,UIActivityTypeSaveToCameraRoll];
...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        UIBarButtonItem *btn = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Done", @"") style:UIBarButtonItemStylePlain target:self action:@selector(doneTapped:)] autorelease];
